				...
				<div class="col-lg-12">
					<ul class="breadcrumb">
                        ...
                        <c:if test="${requestScope.result.cid != null}">
                            <li><a href="${path}/list?cid=${requestScope.result.cid}&page=1">${requestScope.result.cname}</a></li>
                        </c:if>
                        <c:if test="${requestScope.result.bid != null}">
                            <li><a href="${path}/list?bid=${requestScope.result.bid}&page=1">${requestScope.result.bname}</a></li>
                        </c:if>
					</ul>
				</div>
			</div>
						...
						<ul class="pagination">
                            <c:forEach begin="1" end="${requestScope.result.pageNum}" var="page">
                                <li <c:if test="${param.page == page}">class="active"</c:if>>
                                    <a href="${path}/list?<c:if test="${requestScope.result.cid != null}">cid=${requestScope.result.cid}</c:if><c:if test="${requestScope.result.bid != null}">&bid=${requestScope.result.bid}</c:if><c:if test="${param.keyWord != null}">keyWord=${param.keyWord}</c:if>&page=${page}#breadcrumb">${page}</a>
                                </li>
                            </c:forEach>
						</ul>
					</div>
